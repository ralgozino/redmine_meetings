...
#  map.connect 'projects/:id/meetings_settings/:action', :controller => 'meetings_settings'
  match 'projects/:project_id/meetings/:action', :controller => 'meetings', via: [:get, :put, :post, :patch, :delete]
  match 'meetings/:action/:id', :controller => 'meetings', via: [:get, :put, :post, :patch]
